...
class Anime(commands.Cog):
    ...
    async def anilist(self, ctx, *, uname=""):
        if uname == "":
            await ctx.send(
                f"{ctx.author.mention} you need to give a username `v!anilist <username>`"
            )
            return
        ...
        emb = discord.Embed(
            title=f"Anilist Stats for {uname}", description=desc, color=0xFF0055
        )
    ...
    async def MAL(self, ctx, *, uname=""):
        if uname == "":
            await ctx.send(
                f"{ctx.author.mention} you need to give a username `v!MAL <username>`"
            )
            return
...
"""
        ...
        emb = discord.Embed(
            title=f"MAL Stats for {uname}", description=dsc, color=0xFF0055
        )
        await ctx.send(embed=emb)
