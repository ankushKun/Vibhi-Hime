...
class Fun(commands.Cog):
    ...
    async def on_raw_reaction_add(self, reaction):
        if (
            reaction.message_id in deletable_messages
            and reaction.emoji.name == "🗑️"
    ...
    async def pun(self, ctx):
        pun = requests.get(
            "https://sv443.net/jokeapi/v2/joke/Pun?blacklistFlags=nsfw,religious,political,racist,sexist&format=txt"
        )
    ...
    async def nuke(self, ctx, *, msg=""):
        e = discord.Embed(
            title="", description=f"{ctx.author.mention} NUKES {msg}", color=0xFF0055
        )
        e.set_image(
            url="https://i.pinimg.com/originals/47/12/89/471289cde2490c80f60d5e85bcdfb6da.gif"
        )
        await ctx.send(embed=e)
